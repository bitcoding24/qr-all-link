import streamlit as st
from pathlib import Path

# -----------------------------
# 기본 설정
# -----------------------------
st.set_page_config(
    page_title="나의 웹사이트 연결고리",
    page_icon="🔗",
    layout="centered"
)

# -----------------------------
# CSS 디자인
# -----------------------------
st.markdown("""
<style>
.main-title {
    text-align: center;
    font-size: 34px;
    font-weight: 800;
    margin-bottom: 10px;
}

.sub-title {
    text-align: center;
    font-size: 16px;
    color: #666;
    margin-bottom: 35px;
}

.link-card {
    background-color: #ffffff;
    padding: 22px;
    border-radius: 18px;
    box-shadow: 0px 4px 14px rgba(0,0,0,0.08);
    margin-bottom: 18px;
    border: 1px solid #eeeeee;
}

.link-title {
    font-size: 22px;
    font-weight: 700;
    margin-bottom: 8px;
    color: #111827;
}

.link-desc {
    font-size: 15px;
    color: #555;
    margin-bottom: 16px;
    line-height: 1.6;
}

a.custom-button {
    display: block;
    text-align: center;
    background-color: #2563eb;
    color: white !important;
    padding: 13px 18px;
    border-radius: 12px;
    text-decoration: none;
    font-weight: 700;
    font-size: 16px;
}

a.custom-button:hover {
    background-color: #1d4ed8;
}
            
.download-box {
    background-color: #ffffff;
    padding: 22px;
    border-radius: 18px;
    box-shadow: 0px 4px 14px rgba(0,0,0,0.08);
    margin-bottom: 24px;
    border: 1px solid #eeeeee;
}

.download-title {
    font-size: 22px;
    font-weight: 700;
    color: #111827;
    margin-bottom: 8px;
}

.download-desc {
    font-size: 15px;
    color: #374151;
    margin-bottom: 14px;
    line-height: 1.6;
}
</style>
""", unsafe_allow_html=True)

# -----------------------------
# 제목 부분
# -----------------------------
st.markdown('<div class="main-title">🔗 나의 웹사이트 연결고리</div>', unsafe_allow_html=True)
st.markdown(
    '<div class="sub-title">아래 버튼을 누르면 원하는 웹사이트로 이동할 수 있습니다.</div>',
    unsafe_allow_html=True
)

# -----------------------------
# 학사일정 원본 파일 다운로드
# -----------------------------
schedule_file_path = Path("files/school_schedule.zip")

st.markdown("""
<div class="download-box">
    <div class="download-title">📁 분석에 사용한 학사일정 파일 다운로드</div>
    <div class="download-desc">
        아래 버튼을 누르면 분석에 사용한 월별 학사일정 CSV 파일들을 압축 파일로 다운로드할 수 있습니다.
    </div>
</div>
""", unsafe_allow_html=True)
if schedule_file_path.exists():
    with open(schedule_file_path, "rb") as f:
        st.download_button(
            label="📥 학사일정 파일 다운로드",
            data=f,
            file_name="school_schedule.zip",
            mime="application/zip",
            use_container_width=True
        )
else:
    st.warning("학사일정 파일을 찾지 못했습니다. files/school_schedule.zip 경로를 확인하세요.")

# -----------------------------
# 링크 목록
# 여기만 수정하면 버튼이 바뀜
# -----------------------------
links = [
    {
        "title": "+더 많은 학사일정 다운받기+",
        "description": "위 사이트에서 다운받은 학사일정만 사용 가능합니다.",
        "url": "https://open.neis.go.kr/portal/data/service/selectServicePage.do?page=1&rows=10&sortColumn=&sortDirection=&infId=OPEN17220190722175038389180&infSeq=3&cateId=A0005"
    },
    {
        "title": "📚 학사일정 기반 학습 공백 위험지수 대시보드",
        "description": "위 데이터를 다운받으시고, 업로드하면 결과를 보실 수 있습니다. 또한 다른기간의 학사일정을 다운로드 받아도 분석 가능합니다.",
        "url": "https://schedule-blank-risk.streamlit.app/"
    },
    {
        "title": "📚 2025-03-01 ~ 2026-02-28 학사일정 기반 학습 공백 위험지수 결과 바로보기",
        "description": "위 데이터의 다운로드 없이 주제로 정한 기간의 결과를 바로 보실 수 있습니다.",
        "url": "https://schedule-blank-risk-oneshow.streamlit.app/"
    }
    ,
    {
        "title": "전국 학교 망각 위험지수 지도",
        "description": "전국 학교의 망각 위험지수를 지도에 표시하였습니다.",
        "url": "https://forget-risk-map.streamlit.app/"
    },
    {
        "title": "🧬 전국 학교 실제 학사일정 vs GA 최적 학사일정 차이 분석",
        "description": "위 데이터를 다운받으시고, 업로드하면 결과를 보실 수 있습니다. 또한 다른기간의 학사일정을 다운로드 받아도 분석 가능합니다.",
        "url": "https://ga-vs-real-difference-analysis.streamlit.app/"
    },
    {
        "title": "📊 전국 학교 실제 학사일정 vs GA 최적 학사일정 차이 분석 결과 바로보기",
        "description": "위 데이터의 다운로드 없이 주제로 정한 기간의 결과를 바로 보실 수 있습니다.",
        "url": "https://ga-vs-real-differnce-analysis-result.streamlit.app/"
    }
    
    
    
]

# -----------------------------
# 링크 카드 출력
# -----------------------------
for link in links:
    st.markdown(f"""<br>
    <div class="link-card">
        <div class="link-title">{link["title"]}</div>
        <div class="link-desc">{link["description"]}</div>
        <a class="custom-button" href="{link["url"]}" target="_blank">
            이동하기
        </a>
    </div>
    """, unsafe_allow_html=True)

# -----------------------------
# 아래 안내 문구
# -----------------------------
st.markdown("---")
st.caption("QR 코드를 통해 접속한 뒤 원하는 버튼을 눌러 이동할 수 있습니다.")